...
- **ProcessingSettings** (текущие глобальные параметры пайплайна):
  ...
  - Управление параллельностью осуществляется через режим пайплайна (`pipeline_mode`); отдельные числовые лимиты больше не задаются.
...
- Параметры `max_parallel_transcriptions` и `max_parallel_corrections` удалены: параллельность теперь полностью определяется выбранным режимом пайплайна.
- Параметр `correction_device` удалён: использование GPU для коррекции контролируется через параметры LLM (например, `llama_gpu_layers`).
